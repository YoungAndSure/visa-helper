...
选择本地文件夹
  → 原文件只读预览（不修改原文件）
  → 本地识别/预处理：把 PDF、文本和图片转换成统一 Document Context
  → 插件式一级审核：每条规则从完整 context 中自行寻找候选材料并输出判定
  → 点击“擦除隐私并生成安全材料”
  → 规则擦除明显 PII
  → 图片生成待人工处理对象，原图内容保持为空
  → 每个原文件映射一个安全材料对象
  → 用户从左侧逐个选择，通过“文件预览 / 隐私材料”页卡对照检查和编辑
  → 用户逐个确认
  → 发送安全 JSON，运行后端二级审核并生成最终报告
```
...
页面不直接展示整包裸 JSON。下面仅用于说明前后端协议；实际界面按文件拆分，正文按原材料顺序以文本块和图片块连续展示。当前解析器主要生成文本块，图片提取和顺序还原将在后续版本细化。
  ...
  "materials": [
    {
      ...
      "images": [],
      "content_blocks": [
        {"type": "text", "text": "姓名: [REDACTED_NAME]"}
      ],
...
`content_blocks` 是前端展示和后续 Agent 阅读的有序内容流。文本块与图片块按原材料阅读顺序排列；`text` 和 `images` 暂时保留为兼容字段。
