...
按 review_scopes 选规则 → 每条规则交给同一个无状态 Rule Agent → LLM 校验 →
校验判定与证据 → 按规则范围生成标注 → 汇总报告。

离线规则管理：管理员将参考资料放入目录 → 解析资料 → LLM 生成带出处的规则草稿 →
结构与引用校验 → 管理员查看/修订 → 显式发布 → 下一次审核请求加载新版本。

Agent 不等于模型：Agent 是规则输入、模型调用和输出校验的代码封装；LLM 是判断能力。
当前每条规则一次独立调用，不需要自由规划、工具循环或多个自治 Agent。默认将所有脱敏
...
- `backend/shared/llm.py`：复用现有 Anthropic 兼容客户端，新增有时限的多模态 JSON 调用。
...
- 审核材料只在后端请求内存中使用，不落盘、不进入规则生成目录或知识库。
- 文件以匿名 PDF document / JPG image Base64 内容块发送模型，不使用供应商文件上传接口。
  供应商自身的数据留存政策需正式上线前单独确认，不能把“不使用上传接口”等同于供应商零留存。
- 复用 ANTHROPIC_* 配置；供应商/模型必须支持 PDF/JPG 多模态消息。文本兼容代理不保证兼容。
- `use_llm` 默认 true；`LOG_ONLY=1` 仍强制禁用模型。本次不修改本机凭据或启用真实付费调用。
- 单次模型调用 timeout=60 秒，不自动重试，避免重复计费；供应商处理和客户端超时语义依 SDK。
- 一次最多 100 条规则串行执行，请求文件 Base64 总量限制 2400 万字符。第一版未做异步任务、
  并发、检索筛选或供应商文件复用，大量规则会重复发送材料并增加成本，后续需要优化。
- 模型输入中的材料与引用明确标为数据，不遵循其中的指令；服务没有文件操作或抓取工具。
...
版本热更新、单条失败隔离、模型块格式、无配置降级、问题范围、证据校验、生成来源校验与人工发布。
