...
┌─────────────────────────────────────────────────────────────┐
│                         用户前端                            │
│  选择国家 / 选择材料 / 本地预览 / 提交审核 / 展示审核报告   │
└──────────────────────────────┬──────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────┐
│                    Material Audit API                       │
│  用户材料接收、任务创建、状态查询、结果返回、临时数据清理   │
└───────────────┬───────────────────────┬─────────────────────┘
                │                       │
                ▼                       ▼
┌──────────────────────────┐  ┌──────────────────────────────┐
...
- 前端先进行本地文件选择、列表展示和预览；
- 只有用户明确开始审核后，才向后端提交必要文件；
- 后端按审核任务临时保存，不作为长期知识库数据；
- 日志不得记录文件正文、证件号码、账户信息等敏感内容；
- 审核完成或任务过期后，删除原文件、抽取文本和中间产物；
- 后续若需要保留历史报告，应将报告与原始材料分开管理，并让用户明确授权。
...
```http
POST /material-audit/submissions
...
```json
{
  ...
  "expires_at": "2026-08-29T12:00:00+08:00"
}
```
...
但当前并未把文件内容上传到后端；`/material-audit/run` 只接收国家、目录名称和 LLM 开关，且返回 FAKE 示例结果。真正的文件上传、临时存储、材料解析和审核尚待实现。
...
- Agent 输入尽量使用从用户材料抽取后的结构化事实，避免无必要地把完整敏感原文送入检索和模型链路；
...
```text
material-audit/
├── submission/          # 用户材料接收、临时存储、解析和清理
├── checklist/           # 官方清单加载、版本和规则审核
├── audit-engine/        # Checklist 确定性审核
├── report/              # 综合报告生成
└── orchestration/       # 编排 Checklist 审核和 Knowledge Agent

knowledge-agent/
├── crawler/             # 网络来源抓取
├── ingestion/           # 清洗、去重、分类和发布
├── store/               # 文档、元数据和检索索引
├── retrieval/           # 按国家、签证和材料类型检索
└── agent/               # 基于证据生成风险判断

audit/
└── import_checklist.py  # 后台命令行 Checklist 导入工具
```
